...
while temp_new < counter:
    while crit_ask < lensM:
        print("На сколько " + crit_list[temp_new] + " важнее чем " + crit_list[crit_ask])
        try:
            mod = int(input("Введите множители для критериев по важности. 1-неважно. 9-важно: "))
            if mod > 9 or mod < 1:
                print("Числа от 1 до 9")
                continue
            else:
                input_new.append(mod)
                print(input_new)
        except ValueError:
            print("Not a number")
            continue
...
print(dictionary)
best = 0
print("Список от лучшего варианта к худшему")
